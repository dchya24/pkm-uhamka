        ...
        <div class="col-sm">
          <div class="card">
            <div class="card-header">
              <h5 class="pt-3 text-center">Jumlah usulan yang dinilai penilai substansi</h5>
            </div>
            <div class="card-body">
              <div class="">
                <table id="example1" class="table table-bordered table-striped text-center">
                  <thead>
                    <tr class="text-bold">
                      <th>No</th>
                      <th>Nama Penilai Substansi</th>
                      <th>Jumlah Proposal yang dinilai</th>
                      <th>Aksi</th>
                    </tr>
                  </thead>
                  <tbody>
                    @forelse ($penilaiSubstansi as $item)
                        <tr>
                          <td>{{$loop->index + 1}}</td>
                          <td>{{ $item->nama }}</td>
                          <td>
                            @if ($item->penilaianSubstansi->count() == 0)
                              <span class="text-danger">Belum ada proposal yang dinilai</span>
                            @else
                              {{ $item->penilaianSubstansi->count() }}
                            @endif
                          </td>
                          <td>
                            <a href="{{ route('admin.manajemen-proposal.penilai-substansi.tambah', $item->id) }}">
                              <button
                                type="button"
                    ...
                    @empty
                        <tr>
                          <td colspan="4" class="text-bold">Belum ada penilai substansi</td>
                        </tr>
                    @endforelse
                  </tbody>
                </table>
